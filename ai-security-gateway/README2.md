...
**Real numbers from existing prompt-injection classifiers**, which is
what the device-tier thresholds in `device_profile.py` are calibrated
against:

| Model | Parameters | Disk (quantized) | RAM at inference | Latency | Edge fit |
|---|---|---|---|---|---|
| Llama Prompt Guard 2 -- 22M | 22M (DeBERTa-xsmall) | ~25 MB (INT8) | ~50-100 MB | ~19 ms/prompt (CPU) | Raspberry Pi-class or better |
| Llama Prompt Guard 2 -- 86M | 86M backbone + 192M embeddings | ~90 MB (INT8) | ~200-400 MB | ~92 ms/prompt (GPU) | Needs a more capable edge box |
| Llama Guard 3 -- 1B | 1B | ~500-700 MB (INT4) | ~1-1.5 GB | Seconds on weak CPUs | Meta positions this size specifically for edge |

Rule of thumb for sizing any model: `parameters x bytes-per-parameter x 1.3-1.5` for
runtime overhead. INT8 = 1 byte/param, INT4 = 0.5 bytes/param, FP16 = 2, FP32 = 4.
